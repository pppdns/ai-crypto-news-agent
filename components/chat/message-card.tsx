...
export type MessageCardProps = Omit<React.HTMLAttributes<HTMLDivElement>, 'role'> & {
...
const MessageCard = React.forwardRef<HTMLDivElement, MessageCardProps>(
  ({ speaker, message, status, className, ...props }, ref) => {
        ...
        <div className="flex w-10 shrink-0 flex-col items-start sm:w-14">
          <span
            className={cn(
              ...
              speaker === 'assistant' ? 'text-accent' : 'text-muted',
            )}
          >
            {label}
          </span>
          <span
            className={cn('mt-2 w-px flex-1', speaker === 'assistant' ? 'bg-accent/55' : 'bg-hairline-strong')}
            aria-hidden
          />
        </div>
        <div
